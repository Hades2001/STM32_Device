...
{ 
	...
	W25QXX_ID=W25QXX_ReadID();	              //读取W25QXX_ID.
}  
/****************************************************************************
...
{  
	u8 byte=0;   
	GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                                  //使能器件   
	SPI2_ReadWriteByte(W25X_CMD_ReadStatusReg);   //发送读取状态寄存器命令    
	byte=SPI2_ReadWriteByte(0Xff);                //读取一个字节  
	GPIO_SetBits(SPI_W25Q_nCS);//W25QXX_CS=1;                                  //取消片选     
	return byte;   
} 
/****************************************************************************
...
{   
	GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                                 //使能器件   
	SPI2_ReadWriteByte(W25X_CMD_WriteStatusReg); //发送写取状态寄存器命令    
...
{
	GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                               //使能器件   
  SPI2_ReadWriteByte(W25X_CMD_WriteEnable);  //发送写使能  
	GPIO_SetBits(SPI_W25Q_nCS);//W25QXX_CS=1;                               //取消片选     	      
} 
//等待空闲
...
{  
	GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                                //使能器件   
  SPI2_ReadWriteByte(W25X_CMD_WriteDisable);  //发送写禁止指令    
	GPIO_SetBits(SPI_W25Q_nCS);//W25QXX_CS=1;                                //取消片选     	      
} 
/****************************************************************************
...
{                                   
    ...
    W25QXX_Wait_Busy();   
  	GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                             //使能器件   
    SPI2_ReadWriteByte(W25X_CMD_ChipErase);  //发送片擦除命令  
	  GPIO_SetBits(SPI_W25Q_nCS);//W25QXX_CS=1;                             //取消片选     	      
...
{    
    ...
    W25QXX_Wait_Busy();   
  	GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                                  //使能器件   
    SPI2_ReadWriteByte(W25X_CMD_SectorErase);     //发送扇区擦除指令 
    SPI2_ReadWriteByte((u8)((First_Addr)>>16));   //发送地址    
...
{ 
  	GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                             //使能器件   
    SPI2_ReadWriteByte(W25X_CMD_PowerDown);  //发送掉电命令  
	  GPIO_SetBits(SPI_W25Q_nCS);//W25QXX_CS=1;                             //取消片选     	      
...
{  
  	GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                                      //使能器件   
    SPI2_ReadWriteByte(W25X_CMD_ReleasePowerDown);    //发送唤醒命令
	  GPIO_SetBits(SPI_W25Q_nCS);//W25QXX_CS=1;                                      //取消片选     	                                         
}   
...
u8 W25QXX_ReadByte(u32 StartAddress)
{
	  ...
	  GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                                 //使能器件   
    SPI2_ReadWriteByte(W25X_CMD_ReadData);     //发送读取命令   
    SPI2_ReadWriteByte((u8)((StartAddress)>>16));  //发送24bit地址    
...
{  
	  W25QXX_Write_Enable();                   
	  GPIO_ResetBits(SPI_W25Q_nCS);//W25QXX_CS=0;                                   //使能器件   
    SPI2_ReadWriteByte(W25X_CMD_PageProgram);      //发送写页命令   
    SPI2_ReadWriteByte((u8)((StartAddress)>>16));  //发送地址    
    SPI2_ReadWriteByte((u8)((StartAddress)>>8));   
    SPI2_ReadWriteByte((u8)StartAddress);   
		while (ByteCount--)
			{
